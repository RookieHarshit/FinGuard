...
@pytest.mark.asyncio
async def test_send_otp_calls_sms_provider_once(mocker):
    phone = "+919876543210"
...
@pytest.mark.asyncio
async def test_send_otp_generate_otp_failure_propagates(mocker):
    phone = "+919876543210"
    ...
    mocker.patch(
        "app.auth.OTP.service.generate_otp",
        side_effect=RuntimeError("OTP generation failed")
    )
